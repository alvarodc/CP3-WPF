...
namespace CardPass3.WPF.Data.Repositories
{
    public class OperatorRepository : IOperatorRepository
    {
        ...
        public OperatorRepository(IDatabaseConnectionFactory db) => _db = db;

        public async Task<Operator?> GetByNameAsync(string name, CancellationToken ct = default)
        {
            const string sql = @"
                SELECT id_operator AS IdOperator, operator_name AS OperatorName,
                       password AS Password, operator_description AS OperatorDescription
                FROM operators WHERE operator_name = @name";
        ...
        {
            const string sql = @"
                SELECT f.function_name FROM functions f
                INNER JOIN operator_function_assoc ofa ON ofa.functions_id_function = f.id_function
                WHERE ofa.operators_id_operator = @operatorId AND f.active = 1";
            using var conn = await _db.CreateOpenConnectionAsync(ct);
    ...
    {
        private readonly IDatabaseConnectionFactory _db;
        public ConfigurationRepository(IDatabaseConnectionFactory db) => _db = db;

        public async Task<string?> GetValueAsync(string name, CancellationToken ct = default)
        {
            using var conn = await _db.CreateOpenConnectionAsync(ct);
            return await conn.QuerySingleOrDefaultAsync<string?>(
                new CommandDefinition("SELECT value FROM configuration WHERE name = @name",
                    new { name }, cancellationToken: ct));
        }

        public async Task SetValueAsync(string name, string value, CancellationToken ct = default)
        {
            const string sql = "INSERT INTO configuration (name, value) VALUES (@name, @value) ON DUPLICATE KEY UPDATE value = @value";
            using var conn = await _db.CreateOpenConnectionAsync(ct);
        ...
        public async Task<Dictionary<string, string?>> GetAllAsync(CancellationToken ct = default)
        {
            ...
            var rows = await conn.QueryAsync<ConfigurationEntry>(
                new CommandDefinition("SELECT name, value FROM configuration", cancellationToken: ct));
